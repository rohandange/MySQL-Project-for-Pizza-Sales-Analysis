...
SELECT 
    ROUND(SUM(order_details.quantity * pizzas.price),
            2) AS 'Total Revenue Genrated from Pizza Sales'
FROM
    order_details
        JOIN
    pizzas ON order_details.pizza_id = pizzas.pizza_id; -- used ctrl + b to beautify this query only. 

-- Identify the highest-priced pizza. 
select max(price) as "Highest Priced Pizza" from pizzas;

-- above query can also be written as following 
select 
pizza_types.name as "Pizza Name", pizzas.price as "Highest Priced Pizza" 
...
group by pizza_types.name order by order_quantity desc limit 5; 
 ...
 select pizza_types.category as "Pizza Category", sum(order_details.quantity) as Total_Quantity
 from pizza_types join pizzas 
 on pizza_types.pizza_type_id = pizzas.pizza_type_id
 join order_details 
 on order_details.pizza_id = pizzas.pizza_id
